...
function extractLookupValue(v: unknown): string | undefined {
  if (typeof v === "string") return v || undefined;
  if (v !== null && typeof v === "object") {
    const o = v as Record<string, unknown>;
    if ("LookupValue" in o && o.LookupValue != null) return String(o.LookupValue);
...
export const orderService = {
  ...
  findAllByTitle: async (title: string): Promise<Order[]> => {
    ...
    const numeric = clean.replace(/^CL/i, "");
    const variants = [...new Set([clean, numeric, `CL${numeric}`])];
