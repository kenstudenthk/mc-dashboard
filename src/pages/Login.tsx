  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between items-baseline">
        <label className="text-[11px] font-semibold text-[#1d1d1f]/55 tracking-tight">
          {label}
        </label>
...
function PasswordRule({ met, label }: { met: boolean; label: string }) {
  return (
    <div
      className={`flex items-center gap-2 text-[11px] transition-colors duration-200 ${met ? "text-[#078a52] font-semibold" : "text-[#1d1d1f]/40"}`}
    >
      <span
        className={`w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 transition-all duration-200 ${met ? "bg-[#078a52]" : "bg-[#1d1d1f]/10"}`}
      >
    ...
    <div className="hidden md:flex w-[420px] shrink-0 bg-gradient-to-br from-[#252528] to-[#1c1c1e] flex-col p-10 relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 relative">
        <div className="w-10 h-10 flex items-center justify-center shrink-0">
          <svg width="36" height="36" viewBox="0 0 42 42" fill="none">
            <path
              d="M7 29 L7 13 L14 24 L21 13 L21 29"
              stroke="white"
              strokeWidth="2.2"
              strokeLinecap="round"
            ...
            />
            <path
              d="M38 17 C36 12 29 11 26 16 C23 21 25 29 30 30 C33 31 36 29 38 26"
              stroke="rgba(59,211,253,0.90)"
              strokeWidth="2.2"
              strokeLinecap="round"
            />
          </svg>
      ...
      <div className="flex-1 flex flex-col justify-center relative gap-2.5">
        ...
        <p className="text-sm text-white/45 leading-relaxed max-w-[300px] mt-1.5">
          Internal operations tool for multi-cloud account provisioning across
...
function SignInScreen({
  ...
  return (
    <AuthCard>
      <div className="mb-6">
        <h2 className="text-2xl font-extrabold text-[#1d1d1f] leading-tight">
          Sign in
        </h2>
        <p className="text-xs text-[#1d1d1f]/45 mt-1.5">
      ...
      >
        ...
        <div className="flex items-center justify-between mt-0.5">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <span className="text-xs text-[#1d1d1f]/55 font-medium">
              Remember me
            </span>
          </label>
          <button
...
function ChangePasswordScreen({
  ...
  async function handleSubmit(ev: React.FormEvent) {
    ...
    try {
      ...
      await authService.clearForcePasswordChange();
      onDone();
    } catch (err) {
      setErrors({
        next:
  ...
  return (
    <AuthCard>
      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#fef9c3] border border-[#f8cc65] mb-4">
        ...
        <span className="text-[11px] font-bold text-[#a16207] tracking-tight">
          First-time login
        </span>
      </div>
      <h2 className="text-[22px] font-extrabold text-[#1d1d1f] leading-tight mb-1.5">
        Set your password
      </h2>
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-[#f5f4f1] rounded-[9px] border border-[#1d1d1f]/08 mb-5">
        ...
        <span className="text-xs text-[#1d1d1f]/55 font-mono truncate">
      ...
      >
        ...
        {next.length > 0 && (
          <div className="flex flex-col gap-1.5 px-3 py-2.5 bg-[#f9f8f6] rounded-[9px] border border-[#1d1d1f]/07">
            {ruleResults.map((r, i) => (
              <PasswordRule key={i} met={r.met} label={r.label} />
            ))}
          </div>
        )}
        <Field
        ...
        <div className="mt-2">
          <PrimaryBtn
            loading={loading}
            disabled={next.length > 0 && !allRulesMet}
          >
            Set New Password
          </PrimaryBtn>
        </div>
...
function CheckInboxScreen({
  ...
  async function handleResend() {
    try {
      ...
      // Silent fail — user still sees the "resent" confirmation
    }
    setResent(true);
  ...
  return (
    <div className="w-full max-w-[380px] animate-[fadeUp_0.4s_cubic-bezier(0.34,1.4,0.64,1)_both]">
      <div className="card p-8 text-center">
        ...
        <h2 className="text-[22px] font-extrabold text-[#1d1d1f] mb-2">
          Check your inbox
        </h2>
        <p className="text-xs text-[#1d1d1f]/50 mb-1.5 leading-relaxed">
          We sent a password reset link to
        </p>
        <p className="text-sm font-bold text-[#0071e3] mb-6 font-mono break-all">
          {email}
        </p>
        <div className="bg-[#f9f8f6] rounded-[10px] p-3.5 border border-[#1d1d1f]/07 text-left mb-6 flex flex-col gap-2">
          {[
            "Open the email from MC Dashboard",
            'Click the "Reset Password" link',
            "The link expires in 30 minutes",
          ].map((step, i) => (
            <div
              key={i}
              className="flex gap-2.5 text-xs text-[#1d1d1f]/60 leading-snug"
            >
              <span className="w-[18px] h-[18px] rounded-full bg-[#0071e3]/10 text-[#0071e3] flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5">
                {i + 1}
              </span>
              {step}
            </div>
        ...
        <div className="flex items-center justify-center gap-2.5">
          ...
          <button
            onClick={onBack}
            className="text-xs font-semibold text-[#1d1d1f]/45 hover:underline"
          >
            Back to sign in
          </button>
        </div>
...
function SuccessScreen({
  ...
  return (
    <div className="w-full max-w-[360px] text-center animate-[fadeUp_0.4s_cubic-bezier(0.34,1.4,0.64,1)]">
      ...
      <h2 className="text-[26px] font-extrabold text-[#1d1d1f] mb-2">
        All done!
      </h2>
      <p className="text-sm text-[#1d1d1f]/50 mb-7 leading-relaxed">
        {message}
      </p>
      <div className="flex items-center justify-center gap-2">
        <RefreshCw className="w-3.5 h-3.5 text-[#1d1d1f]/30 animate-spin" />
        <span className="text-xs text-[#1d1d1f]/35 font-medium">
          Loading dashboard…
        </span>
      </div>
    </div>
...
export default function Login({
  ...
  // When the user clicks the reset-password email link, Supabase redirects them
  // back to /login with a recovery token in the URL hash. detectSessionInUrl in
  // the Supabase client exchanges it automatically and fires PASSWORD_RECOVERY.
  useEffect(() => {
    const {
      data: { subscription },
    } = supabase.auth.onAuthStateChange((event) => {
  ...
  function renderScreen() {
    switch (screen) {
      ...
      case "change-password":
        return (
          <ChangePasswordScreen email={firstTimeEmail} onDone={handleSuccess} />
        );
      case "forgot-email":
        return (
          <ForgotEmailScreen
      ...
      case "reset-password":
        return <ResetPasswordScreen onDone={handleSuccess} />;
      case "success":
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div
        key={screen}
        className="flex-1 flex items-center justify-center px-10 py-16 overflow-y-auto bg-[#faf9f7]"
      >
        {renderScreen()}
      </div>
    </div>
